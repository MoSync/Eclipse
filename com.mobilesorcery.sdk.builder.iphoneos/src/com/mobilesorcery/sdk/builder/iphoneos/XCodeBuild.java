...
public class XCodeBuild extends AbstractTool {
	...
	/**
	 * Returns the xcodebuild tool with the default path (ie it is
	 * expected to be added to the system path, and modified using the
	 * xcode-select command)
	 * @return
	 */
	 ...
	 * @param sdk The sdk to use for building, or <code>null</code> for the default.
	 * @return The path to the built app
	 * @throws CoreException If the build for some reason failed.
	 */
	public void build(IPath pathToXcodeProject, String target, String sdk) throws CoreException {
		...
		if (execute(cmd.toArray(new String[cmd.size()]), null, null, false) != 0) {
			throw new CoreException(new Status(IStatus.ERROR, "com.mobilesorcery.sdk.builder.iphoneos", "xcodebuild failed to build"));
		}
	}
	...
	/**
	 * Returns the list of available SDKs, or <code>null</code> if unable to extract them.
	 * @return
	 */
